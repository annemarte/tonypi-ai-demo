...
def read_temperature_humidity(bus=1, addr=0x38):
    """Read temperature (in Celsius) and humidity (in %) from the AHT10 sensor.

    Returns:
        tuple: (temperature, humidity)
    """
    aht10 = AHT10(bus=bus, addr=addr)
    return aht10.getData()
